...
import dac
from dac.model import HiddenQuantizer
...
# Models
ResidualVectorQuantize = argbind.bind(dac.nn.quantize.ResidualVectorQuantize)
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "quantizer").exists():
            quantizer, extra = ResidualVectorQuantize.load_from_folder(**kwargs)

    if quantizer is None:
        quantizer = ResidualVectorQuantize()

    dac_model = dac.model.DACNoVq.load(dac_path)
    model = HiddenQuantizer(quantizer=quantizer, dac=dac_model)
    ...
    with argbind.scope(args):
        optimizer = AdamW(model.quantizer.parameters(), use_zero=accel.use_ddp)
...
def val_loop(batch, state, accel, lambdas):
    ...
    output = {
        "vq/hidden_loss": out["vq/hidden_loss"],
...
def train_loop(state, batch, accel, lambdas):
    ...
    with accel.autocast():
        ...
        output["vq/hidden_loss"] = out["vq/hidden_loss"]
    ...
    output["other/grad_norm"] = torch.nn.utils.clip_grad_norm_(
        state.model.quantizer.parameters(), 1e3
    )
...
def checkpoint(state, save_iters, save_path):
    ...
    for tag in tags:
        ...
        accel.unwrap(state.model.quantizer).metadata = metadata
        accel.unwrap(state.model.quantizer).save_to_folder(
