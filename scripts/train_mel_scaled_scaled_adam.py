...
# Optimizers
ScaledAdam = argbind.bind(dac.optim.ScaledAdam, "generator", "discriminator")
Eden2 = argbind.bind(dac.optim.Eden2, "generator", "discriminator")
Accelerator = argbind.bind(ml.Accelerator, without_prefix=True)

# Models
DACNoWn = argbind.bind(dac.model.DACNoWn)
DiscriminatorNoWn = argbind.bind(dac.model.DiscriminatorNoWn)
...
class State:
    generator: DACNoWn
    optimizer_g: ScaledAdam
    scheduler_g: Eden2

    discriminator: DiscriminatorNoWn
    optimizer_d: ScaledAdam
    scheduler_d: Eden2
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "dacnown").exists():
            generator, g_extra = DACNoWn.load_from_folder(**kwargs)
        if (Path(kwargs["folder"]) / "discriminatornown").exists():
            discriminator, d_extra = DiscriminatorNoWn.load_from_folder(**kwargs)

    generator = DACNoWn() if generator is None else generator
    discriminator = DiscriminatorNoWn() if discriminator is None else discriminator
    ...
    with argbind.scope(args, "generator"):
        optimizer_g = ScaledAdam(generator.named_parameters())
        scheduler_g = Eden2(optimizer_g)
    with argbind.scope(args, "discriminator"):
        optimizer_d = ScaledAdam(discriminator.named_parameters())
        scheduler_d = Eden2(optimizer_d)
