...
# Models
DACAddNoise = argbind.bind(dac.model.DACAddNoise)
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "dac").exists():
            generator, g_extra = DACAddNoise.load_from_folder(**kwargs)
        if (Path(kwargs["folder"]) / "discriminator").exists():
            discriminator, d_extra = Discriminator.load_from_folder(**kwargs)

    generator = DACAddNoise() if generator is None else generator
...
def val_loop(batch, state, accel):
    ...
    batch_rms_loss = state.batch_rms_loss(z)
